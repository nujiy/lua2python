...
def parse_nodes(lua_nodes, ctx_klass=py_ast.Load):
    ...
    while len(lua_nodes) > 0:
        ...
        if isinstance(node, astnodes.Block):
            ...
            wrap_expr = lambda x: (
                py_ast.Expr(value=x) if isinstance(x, py_ast.Call) else x
            )
            out.append([wrap_expr(elem) for elem in block])
        ...
        if isinstance(node, astnodes.Assign):
            ...
            out.append(py_ast.Assign(targets=target_expr, value=value_expr))
            continue
        ...
        if isinstance(node, astnodes.LoOp):
            ...
            out.append(py_ast.BoolOp(op=ops_ref, values=[arg_left, arg_right]))
            continue

        if isinstance(node, astnodes.ULNotOp):
            processed_nodes.add(id(node))
            operand = parse_nodes([node.operand])
            out.append(py_ast.UnaryOp(op=py_ast.Not(), operand=operand))
            continue
                ...
                py_ast.FunctionDef(
                    ...
                    args=py_ast.arguments(
                        args=[
                            ...
                            for x in node.args
                            if not processed_nodes.add(id(x)) or True
                        ],
                        vararg=None,
                        kwonlyargs=[],
                        kw_defaults=[],
        ...
        if isinstance(node, astnodes.Return):
            processed_nodes.add(id(node))
            out.append(py_ast.Return(value=parse_nodes(node.values)))
            continue

        if isinstance(node, astnodes.Name):
            processed_nodes.add(id(node))
            out.append(py_ast.Name(id=node.id, ctx=ctx_klass()))
            continue
    return out
